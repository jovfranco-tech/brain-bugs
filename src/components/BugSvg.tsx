...
function Accessory({ id, animFloat = false }: { id: string; animFloat?: boolean }) {
  const content = (() => {
    if (id === 'crown') {
      return (
  ...
  return (
    <g className={animFloat ? "animate-accessory-float origin-center" : ""}>
      {content}
    </g>
  );
}

export default function BugSvg({ kind, size = 80, className = '', animated = false, accessoryId = null, expression, accessoryFloat = false }: BugSvgProps) {
  ...
  if (kind === 'pip') return (
    <svg width={size} height={size} viewBox="0 0 100 100" className={`${anim} ${className}`}>
      ...
      {accessoryId && <Accessory id={accessoryId} animFloat={accessoryFloat}/>}
    </svg>
  );
